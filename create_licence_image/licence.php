        	...
        	<div id="licence" class="tab-pane fade  <?php if(!isset($_SESSION['licence_section']) && $_SESSION['licence_section'] != 'card_list' && $_SESSION['licence_section'] != 'edit_licence') echo 'in active'; else echo 'fade'; ?>">
                <form method="post" action="model/licence_model.php" enctype="multipart/form-data">
                    ...
                    <div class="form-inline" style="margin: 0 0 20px;">
                        <div class="form-group"><label for="text">Licence No:</label><input id="text-short" name="licence_no" required="required" type="text" class="form-control licence_no" value="<?php echo isset($licence_data->licence_no) ? $licence_data->licence_no + 1 : '1'; ?>"/></div>
                        <div class="form-group"><label for="date">Place of Issue:</label><input id="text-short" name="issue_place" type="text" class="form-control issue_place" value="Mogadishu"/></div>
                        <div class="form-group"><label for="date">Date of Issue:</label><input id="text-short" name="issue_date" onkeyup="Change_expire_date(this.value)" required="required" type="text" class="form-control licence_receipt_issueDate" placeholder="dd-mm-yy"/></div>
                        <div class="form-group"><label for="date">Expire Date:</label><input id="text-short" name="expiry_date" required="required" type="text" class="form-control expire_date" placeholder="dd-mm-yy"/></div>
                    </div>
                    <script>
                            ...
                            <div class="col2">
                                <div class="form-horizontal" style="margin: 0 0 20px;">
                                    ...
                                    <div class="form-group">
                                    	...
                                    	<div class="col-sm-4"><input id="text-middle" type="text" name="birth_date" required="required" class="form-control"/></div>
                                    </div>
                                    <div class="form-group">
                                        <label for="text" class="control-label2 col-sm-4">Place of Birth:</label>
                                        <div class="col-sm-4"><input id="text-middle" type="text" name="birth_place" class="form-control"/></div>
                                    </div>
                                    <div class="form-group">
                                     ...
                                     <div class="form-group" style="margin-bottom:10px;"><label for="text" class="control-label2 col-sm-4">Upload Picture:</label>
                                         <div class="col-sm-4">
                                         	<label for="fileinput" class="btn btn-lg btn-default"><span class="glyphicon glyphicon-plus"> Upload</span></label>
                 		   					<input id="fileinput" name="image" type="file" accept="image/*" name="csv_file_for_receipts"  required class="form-control inputfile" style="padding: 0px;" />
                                         </div>
                    ...
                    <div class="form-horizontal">
                        <div class="form-group">
                            ...
                            <div class="col4">
                                <a href="javascript:void(0);" class="btn btn-lg btn-default" onclick="submit_form_btn()"><span class="glyphicon glyphicon-plus"> Add</span></a>
                            </div>
                        </div>
                    </div>
                    <script>
            ...
            <table class="table table-bordered">
				...
				?>
                	<tr>
                        ...
                        <td><a href="?licence_search_id=<?php echo $rec->id; ?>" class="fa fa-print"></a></td>
                   </tr>
                <?php }
				   }else{
...
<script> 
    $(document).ready(function(e) {
        <?php if(isset($_GET['print_licence_no'])){ ?>
				...
				<?php if($_SESSION['is_licence_ready']== 1){ ?> 
				      window.open('licence_print.php', '_blank');
				   //  $('#show_print_btn').show().html('<button onclick="getImages()"> Print License Image </button>');
					
				<?php $_SESSION['is_licence_ready'] = 0; 
				  }else{ ?>	
					$.ajax({
						url:"create_licence_image/index_p.php",
						type:'POST',
						data:'',
						beforeSend: function(){
							$('#show_error_msg').show().html('Loading data, Please wait...');
						},
						success:function(result){ 
						   <?php $_SESSION['is_licence_ready'] = 1; ?>
						    //Popup('<img src="create_licence_image/Licence.jpg" style="border:1px solid #333;margin:20px;"/>');
							//window.open('create_licence_image/Licence.jpg', 'Report', 'height=600,width=600').print();
							
       						
							//$("#Licence_image_card > img").attr('src', 'create_licence_image/Licence.jpg');
							window.location.reload();
							
							//PrintReports('#tab');
						   // $('#Licence_image_card').html('<img src="create_licence_image/Licence.jpg" style="border:1px solid #333;margin:20px;"/>');
							//$('#show_error_msg').hide();
							
							// $('#download_licence_image').trigger('click');
						}
					});
				<?php } ?>	
	    <?php }
