...
// Ajax calls for search...
var s_v_name = '', s_v_number = '', v_type = '', v_contact = '', v_chassis = '', vehicle_records = '', vehicle_records_pagination = '';
...
function get_v_record_by_v_chassis(val)
{
	v_chassis = val;
	get_vehicle_records();
}
...
function get_vehicle_records()
{
	...
	if(s_v_name != '' || s_v_number != '' || v_type != '' || v_contact != '' || v_chassis != '')
	{   // s_v_name  s_v_number s_v_nationality
		$.ajax({
			url:"model/vehicles_model.php",
			type:'POST',
			data:'s_v_name='+s_v_name+'&s_v_number='+s_v_number+'&v_type='+v_type+'&v_contact='+v_contact+'&v_chassis='+v_chassis,
			beforeSend: function(){
...
/* ----------------------------------------------------------------------------------
   **********************************************************************************
   							       Vehicles Transfer MODULE
   **********************************************************************************
   ----------------------------------------------------------------------------------
*/
// Ajax Pagination
function vehicles_transfer_pagination(page)
{
	$.ajax({
		url: 'model/vehicles_transfer_model.php',
		type:'POST',
		data: 'action=vehicles_transfer_ajax_pagination&page='+page,
		beforeSend: function(){
			$('#vechicle_records_area').html('<tr><td colspan="10" align="center" style="color:red;"> Loading..........</td></tr>');
		},
		success: function(result){
...
// Ajax calls for search...
var s_vt_name = '', s_vt_number = '', vt_type = '', vt_contact = '', vehicle_transfer_records = '', vehicle_transfer_records_pagination = '';
function get_vt_record_by_number(val)
{
	s_vt_number = val;
	get_vehicle_transfer_records();
}
function get_vt_record_by_name(val)
{
	s_vt_name = val;
	get_vehicle_transfer_records();
}
function get_vt_record_by_vt_type(val)
{
	vt_type = val;
	get_vehicle_transfer_records();
}
function get_vt_record_by_vt_contact(val)
{
	vt_contact = val;
	get_vehicle_transfer_records();
}

// ajax call for vehical transfer search
function get_vehicle_transfer_records()
{
	if(vehicle_transfer_records == '')
	{
	    vehicle_transfer_records = $('#vechicle_records_area').html();
		vehicle_transfer_records_pagination = $('#pagination_area_vehicle').html();
	}
	if(s_vt_name != '' || s_vt_number != '' || vt_type != '' || vt_contact != '')
	{   // s_v_name  s_v_number s_v_nationality
		$.ajax({
			url:"model/vehicles_transfer_model.php",
			type:'POST',
			data:'s_vt_name='+s_vt_name+'&s_vt_number='+s_vt_number+'&vt_type='+vt_type+'&vt_contact='+vt_contact,
			beforeSend: function(){
				$('#pagination_area_vehicle').html('');
				$('#vechicle_records_area').html('<tr><td colspan="10" align="center" style="color:red;"> Loading..........</td></tr>');
			},
	...
	else
	{
		if(vehicle_transfer_records != '')
		{
		 	// remove session data from search statement...
				...
				success:function(result){
					$('#vechicle_records_area').html(vehicle_transfer_records);
					$('#pagination_area_vehicle').html(vehicle_transfer_records_pagination);
				}
			});
...
// vehicle transfer Card list Ajax Pagination
function vehicles_transfer_cl_pagination(page)
{
	$.ajax({
		url: 'model/vehicles_transfer_model.php',
		type:'POST',
		data: 'action=vehicles_transfer_cl_ajax_pagination&page='+page,
		beforeSend: function(){
			$('#vechicle_card_list_records_area').html('<tr><td colspan="10" align="center" style="color:red;"> Loading..........</td></tr>');
		},
		success: function(result){
			var data = $.parseJSON(result);
			if(data.result != '')
			    $('#vechicle_card_list_records_area').html(data.result);
			if(data.pagination != '')
			    $('#pagination_card_list_area_vehicle').html(data.pagination);
		}
	});
}
/* ----------------------------------------------------------------------------------
...
function get_reports_records()
{
	...
	if(s_report_from_date != '' && s_report_to_date != '' && reports.length > 0)
	{   // s_v_name  s_v_number s_v_nationality
		...
		if($.inArray( '1', reports ) >= 0)
		{
			...
			$.ajax({
				...
				beforeSend: function(){
					...
					$('#receipt_details_account_names_loading_msg').show();
				},
				success:function(result){ 
					var data = $.parseJSON(result);
					$('#receipt_details_account_names_loading_msg').hide();
					if(data.result_daily_report == '')
		...
		if($.inArray( '2', reports ) >= 0)
		{
			...
			beforeSend: function(){
				...
				$('#receipt_details_group_account_names_loading_msg').show();
			},
			success:function(result){ 
				var data = $.parseJSON(result);
				$('#receipt_details_group_account_names_loading_msg').hide();
				if(data.result_daily_report == '')
		...
		if($.inArray( '3', reports ) >= 0)
		{
			...
			beforeSend: function(){
				$('#show_error_msg').show().html('Loading Receipt Report  grouped by account names,.....');
				$('#receipt_details_summry_account_names_loading_msg').show();
			},
			success:function(result){ 
				var data = $.parseJSON(result);
				$('#receipt_details_summry_account_names_loading_msg').hide();
				if(data.result_daily_report == '')
					$('#receipt_detail_grouped_account_name_area').html('<tr><td colspan="10" align="center" style="color:red;"> No record found</td></tr>');
				else
				{  
		...
		if($.inArray( '4', reports ) >= 0)
		{
			...
			beforeSend: function(){
				$('#show_error_msg').show().html('Loading Report,.....');
				$('#receipt_summary_day_loading_msg').show();
			},
			success:function(result){ 
				var data = $.parseJSON(result);
				$('#receipt_summary_day_loading_msg').hide();
				if(data.result_daily_report == '')
		...
		if($.inArray( '5', reports ) >= 0)
		{
			...
			beforeSend: function(){
				$('#show_error_msg').show().html('Loading Report,.....');
				$('#receipt_summary_main_accounts_loading_msg').show();
			},
			success:function(result){ 
				var data = $.parseJSON(result);
				$('#receipt_summary_main_accounts_loading_msg').hide();
				if(data.result == '')
		...
		if($.inArray( '6', reports ) >= 0)
		{
			...
			beforeSend: function(){
				$('#show_error_msg').show().html('Loading Monthly Report,.....');
				$('#receipt_report_summary_month_loading_msg').show();
			},
			success:function(result){ 
				var data = $.parseJSON(result);
				$('#receipt_report_summary_month_loading_msg').hide();
				if(data.result == '')
		...
		if($.inArray( '7', reports ) >= 0)
		{
			...
			beforeSend: function(){
				$('#show_error_msg').show().html('Loading Report,.....');
				$('#receipt_summary_main_sub_accounts_loading_msg').show();
			},
			success:function(result){ 
				var data = $.parseJSON(result);
				$('#receipt_summary_main_sub_accounts_loading_msg').hide();
				if(data.result == '')
		...
		if($.inArray( '8', reports ) >= 0)
		{
			// Users activity report
			$.ajax({
			url:"model/reports_model.php",
			type:'POST',
			data:'action=users_activity_report&s_report_from_date='+s_report_from_date+'&s_report_to_date='+s_report_to_date,
			beforeSend: function(){
				$('#show_error_msg').show().html('Loading Report,.....');
				$('#users_activity_report_loading_msg').show();
			},
			success:function(result){ 
				var data = $.parseJSON(result);
				$('#users_activity_report_loading_msg').hide();
				if(data.result == '')
					$('#users_activity_report_area').html('<tr><td colspan="10" align="center" style="color:red;"> No record found</td></tr>');
				else
				{  
					$('#users_activity_report_area').html(data.result);
					//$('#daily_report_sum').html(data.daily_report_sum);
					$('#users_activity_report #report_period').html(data.top_details);
					$('#users_activity_report_icon').show();
					$('#show_error_msg').hide().html('');  
	...
	else
	{
		if(reports.length == 0)
		{
			$('#show_error_msg').show().html('Please Select reports.');
		}
		else
		{
...
<?php } ?>	
<?php if(isset($_GET['section']) && $_GET['section'] == 'vehicle_search' && isset($_GET['search_vehicle_no'])) { ?>	
      // 
...
var s_l_name = '', s_licence_no = '', s_l_nationality = '', s_l_contact = '', licence_records = '', licence_records_pagination = '', s_l_mother = '';
...
function get_l_record_by_mother(val)
{
	s_l_mother = val;
	get_licence_records();
}
...
function get_licence_records()
{
	...
	if(s_licence_no != '' || s_l_name != '' || s_l_nationality != '' || s_l_contact != '' || s_l_mother != '')
	{   // s_licence_no  s_l_name s_l_nationality
		$.ajax({
			url:"model/licence_model.php",
			type:'POST',
			data:'s_licence_no='+s_licence_no+'&s_l_name='+s_l_name+'&s_l_nationality='+s_l_nationality+'&s_l_contact='+s_l_contact+'&s_l_mother='+s_l_mother,
			beforeSend: function(){
...
var l_from_date = '', l_to_date = '', l_license_no = '', licence_card_records = '', licence_card_records_pagination = '';
...
function get_l_record_by_license_no(val)
{ 
	l_license_no = val;
	get_licence_card_records();
}
...
function get_licence_card_records()
{
	...
	if((l_from_date != '' && l_to_date != '') || l_license_no)
	{   // l_from_date  l_to_date
		$.ajax({

			url:"model/licence_model.php",
			type:'POST',
			data:'l_from_date='+l_from_date+'&l_to_date='+l_to_date+'&l_license_no='+l_license_no,
			beforeSend: function(){
...
//vehicle validate 20March2017
function get_receipt_data_vehicle(receipt_no)
{   
	if(receipt_no != '')
	{
		$.ajax({
...
function get_receipt_data_vehicle_renew(receipt_no)
{   
	if(receipt_no != '')
	{
		$.ajax({
...
{ 
	...
	{
	    vehicle_card_list_records = $('#vechicle_card_list_records_area').html();
		vehicle_card_list_records_pagination = $('#pagination_card_list_area_vehicle').html();
	}
	...
	{   // s_v_card_from_date  s_v_card_to_date
	    $.ajax({
			...
			beforeSend: function(){
				$('#pagination_card_list_area_vehicle').html('');
				$('#vechicle_card_list_records_area').html('<tr><td colspan="12" align="center" style="color:red;"> Loading..........</td></tr>');
			},
			success:function(result){
				var data = $.parseJSON(result);  
				if(data.result == '')
					$('#vechicle_card_list_records_area').html('<tr><td colspan="12" align="center" style="color:red;"> No record found</td></tr>');
				else
				{
					$('#vechicle_card_list_records_area').html(data.result);
					$('#pagination_card_list_area_vehicle').html(data.pagination);
	...
	else
	{
		if(vehicle_card_records != '')
		{
		 	// remove session data from search statement...
				...
				success:function(result){
					$('#vechicle_card_list_records_area').html(vehicle_card_list_records);
					$('#pagination_card_list_area_vehicle').html(vehicle_card_list_records_pagination);
				}
			});
...
// Vehivle new plate number
function get_veh_new_plate_number(v_code)
{ 
	if(v_code=="")
	{	
		$('.plate_no').val("");
	...
	$.ajax({
		url: 'model/vehicles_model.php',
		type:'POST',
		data: 'action=get_new_vehicle_plate_number&code='+v_code,
		beforeSend: function(){
			$('#vechicle_t_records_area').html('<tr><td colspan="10" align="center" style="color:red;"> Loading..........</td></tr>');
		},
		success: function(result){
			var data = $.parseJSON(result);
			//alert(data.NewPlateNo);
			Plate_no_digits = data.plate_no_digits;
			if(data.status == 'yes')
			{ 
			    if(data.NewPlateNo == null)
				{
					if(data.plate_no_digits == 4)   // #uplate_no
						$('.plate_no').val("0001");
					else if(data.plate_no_digits == 3)
						$('.plate_no').val("001");
					else if(data.plate_no_digits == 2)	
					    $('.plate_no').val("01"); 	
				}
				else
				{					
					if((data.NewPlateNo).length == 3)	
					{	
						if(data.plate_no_digits == 4)
							$('.plate_no').val("0"+ data.NewPlateNo);
						else
						    $('.plate_no').val( data.NewPlateNo); 	
					}
					else if((data.NewPlateNo).length == 2 )	
					{	
						if(data.plate_no_digits == 4)
							$('.plate_no').val("00"+ data.NewPlateNo);
						else
						    $('.plate_no').val("0"+ data.NewPlateNo);  	
					}
					else if((data.NewPlateNo).length == 1)	
					{	
						if(data.plate_no_digits == 4)
							$('.plate_no').val("000"+ data.NewPlateNo);
						else
						    $('.plate_no').val("00"+ data.NewPlateNo); 	
					}
					else
				...
				// New code for max digit allow
				if(data.plate_no_digits != '')	
				{	
					$('.plate_no').attr("maxlength" , data.plate_no_digits);
				} 
			}
...
//  Update Vehivle new plate number
function get_veh_new_plate_number_update(v_code)
{ 
	if(v_code=="")
	{	
		$('#uplate_no').val("");
		...
		success: function(result){
			...
			{ 
				...
				{					
					...
					{	
						if(data.plate_no_digits == 4)
							$('#uplate_no').val("0"+ data.NewPlateNo);
						else
						    $('#uplate_no').val( data.NewPlateNo); 	
					}
					else if((data.NewPlateNo).length == 2 )	
					{	
						if(data.plate_no_digits == 4)
							$('#uplate_no').val("00"+ data.NewPlateNo);
						else
						    $('#uplate_no').val("0"+ data.NewPlateNo);  	
					}
					else if((data.NewPlateNo).length == 1)	
					{	
						if(data.plate_no_digits == 4)
							$('#uplate_no').val("000"+ data.NewPlateNo);
						else
						    $('#uplate_no').val("00"+ data.NewPlateNo); 	
					}
					else
				...
				if(data.plate_no_digits != '')	
				{	
					$('#uplate_no').attr("maxlength" , data.plate_no_digits);
				} 
			}
...
// THIS FUNCTION WILL AUTOMATICALLY ADD SLASHES(-) 
var isShift = false;
var seperator = "-";
