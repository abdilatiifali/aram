...
{   
 // licence_no  name nationality date_birth issue_place issue_date expiry_date vehicle_types
	// get last record
	//$sql = 'select licence_no  from tbl_driver_detail order by id desc limit 1';
	$sql = 'select max(licence_no) licence_no  from tbl_driver_detail';
	$record = mysqli_query($con, $sql);
	$last_licence_no = mysqli_fetch_object($record);
	//echo $last_licence_no->licence_no; exit;
...
{ 
	// Check license record already exists or not..
	$sql = 'select *  from tbl_driver_detail where receipt_no = "'.$_POST['receipt_no'].'"  and  name = "'.$_POST['name'].'" and date_birth = "'.$_POST['birth_date'].'"';
	$check_record = mysqli_query($con, $sql);
	if(mysqli_num_rows($check_record) > 0) 
	{
		...
		// Upload image
		$file_name = '';
		//echo '<br/> Last License no : '.$last_licence_no->licence_no.' <br/>';
		if(isset($_FILES["image"]["name"]))
		{
			...
			$ext = pathinfo($_FILES["image"]["name"], PATHINFO_EXTENSION);
			//$file_name = time().'_'.$_FILES["image"]["name"];
			$file_name = ($last_licence_no->licence_no + 1).'.'.$ext;
			@move_uploaded_file($_FILES["image"]["tmp_name"], $target_dir .$file_name);
		...
		$sql = 'INSERT into tbl_driver_detail
				(receipt_no, fee, name, licence_no, issue_place, issue_date, expiry_date, gender, mother_name, 
				 date_birth, birth_place, nationality, address, email, contact_no, personal_id, vehicle_types, comments,image, created_at, username, status, blood_group)
				VALUES
				("'.mysqli_real_escape_string($con, $_POST['receipt_no']).'",
				 ...
				 "'.$_SESSION['USERNAME'].'", 
				 1,
				 "'.mysqli_real_escape_string($con, $_POST['blood_group']).'"
				 )'; 
...
else if(isset($_POST['action']) && $_POST['action'] == 'licence_ajax_pagination_card' && isset($_POST['page']))
{
	...
	if(isset($_SESSION['search_licence_card_statement']) && $_SESSION['search_licence_card_statement'] != '')
		...
		$statement = $pagination_data['current_statement'];
	if(isset($_SESSION['total_pagination_count_card_list']) && $_SESSION['total_pagination_count_card_list'] != '')
	    $record_count = $_SESSION['total_pagination_count_card_list'];
	else
	    $record_count =	$pagination_data['total_pagination_count'];	
	
	//$statement = $pagination_data['current_statement']; // Change `records` according to your table name.
	//$con = $pagination_data['db_con'];
	$data = mysqli_query($con,"{$statement} LIMIT {$startpoint} , {$per_page}");		
	$result = '';	
	$user_privileges = $_SESSION['logged_in_user_privileges'];

	while($rec = mysqli_fetch_object($data))
	{
		   if(isset($user_privileges[5]) && ($user_privileges[5]['e'] == 1 ))
		   {
			   $edit_btns = '<td><a href="?cardId='.base64_encode($rec->id).'" class="glyphicon glyphicon-edit"></a></td>
	...
	$result .= $record_count;
	$paginaton = ajax_pagination_Card_list($record_count,$per_page,$page,$url='licence_card_pagination', $con);
	echo json_encode(array('result' => $result , 'pagination' => $paginaton));
}
...
else if(isset($_POST['s_licence_no']) || isset($_POST['s_l_name']) || isset($_POST['s_l_nationality']) || isset($_POST['s_l_contact']) || isset($_POST['s_l_mother']))      // Search records
{
	$where = '';
	...
	if(isset($_POST['s_l_mother']) && $_POST['s_l_mother'] != '')
	{
	   if($where != '')
	      $where .=' AND ';	
	   $where .= ' mother_name LIKE "%'.mysqli_real_escape_string($con, $_POST['s_l_mother']).'%" ';  
	}
...
else if((isset($_POST['l_from_date']) && $_POST['l_from_date'] != '' && isset($_POST['l_to_date']) && $_POST['l_to_date'] != '') || isset($_POST['l_license_no']))      // Search records
{
	$where = 'd.status = 1 ';
	if((isset($_POST['l_from_date']) && $_POST['l_from_date'] != '' && isset($_POST['l_to_date']) && $_POST['l_to_date'] != ''))
	{
		if($where != '')
		   $where.= ' AND ';
		$where .= ' ( d.issue_date Between "'.date('Y-m-d', strtotime($_POST['l_from_date'])).'" and "'.date('Y-m-d', strtotime($_POST['l_to_date'])).'" 
					  OR (l_r.renewal_type > 0 
					  	 and 
						 (d.updated_At Between "'.date('Y-m-d', strtotime($_POST['l_from_date'])).'" and "'.date('Y-m-d', strtotime($_POST['l_to_date'])).'"))
					 )	 
					 ';
	}
	if(isset($_POST['l_license_no']) && $_POST['l_license_no'] != '')
	{
		if($where != '')
		   $where.= ' AND ';
		$where .= ' d.licence_no LIKE "'.mysqli_real_escape_string($con, $_POST['l_license_no']).'" ';  
	}
	$result = '';
	// statement for pagination
	//$statement = ' tbl_driver_detail where'.$where.' order by id desc';
	...
	$per_page = 10; // Set how many records do you want to display per page.
	$startpoint = ($page * $per_page) - $per_page;
	$sql = $statement.'  LIMIT '.$startpoint.' , '.$per_page.' ';
	//echo $sql; exit;
	$data = mysqli_query($con, $sql);
	$user_privileges = $_SESSION['logged_in_user_privileges'];
	...
	$_SESSION['total_pagination_count_card_list'] = mysqli_num_rows($data_pagination);
	$paginaton = ajax_pagination_Card_list(mysqli_num_rows($data_pagination),$per_page,$page,$url='licence_card_pagination', $con);
	echo json_encode(array('result' => $result, 'pagination' => $paginaton));
} 
// Delete licence
