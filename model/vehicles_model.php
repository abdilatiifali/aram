...
{   ////                     //            
	$sql = "update  tbl_vehicles
			 Set
			  ...
			  status = 1
			  where
			   plate_no = '".(mysqli_real_escape_string($con, $_GET['cardlist_id']))."'
			 ";
	if(mysqli_query($con, $sql))
	{
	     // Update Carlist table
		 $sql = "update  tbl_v_cardlist 
		         SET
				  owner = '".mysqli_real_escape_string($con, $_POST['cl_owner'])."',
				  date_issue = '".date('Y-m-d', strtotime(mysqli_real_escape_string($con, $_POST['cl_issue_date'])))."',
				  expire_date = '".date('Y-m-d', strtotime(mysqli_real_escape_string($con, $_POST['cl_expire_date'])))."',
				  plate_no = '".mysqli_real_escape_string($con, $_POST['cl_plate_no'])."',
				  origin = '".mysqli_real_escape_string($con, $_POST['cl_origin'])."',
				  vehicle_Type = '".mysqli_real_escape_string($con, $_POST['cl_v_type'])."',
				  color = '".mysqli_real_escape_string($con, $_POST['cl_color'])."',
				  chassis_no = '".mysqli_real_escape_string($con, $_POST['cl_chassis_no'])."',
				  model = '".mysqli_real_escape_string($con, $_POST['cl_model'])."',
				  place_issued = '".mysqli_real_escape_string($con, $_POST['cl_issue_place'])."',
				  nationality = '".mysqli_real_escape_string($con, $_POST['cl_nationality'])."',
				  passengers = '".mysqli_real_escape_string($con, $_POST['cl_passengers'])."'  
				WHERE 
					plate_no ='".(mysqli_real_escape_string($con, $_GET['cardlist_id']))."'
...
else if(isset($_POST['add_new_vehicle']))
{
	...
	$_SESSION['passengers'] =  mysqli_real_escape_string($con, $_POST['passengers']);
	...
	{			
		$sql = "INSERT into tbl_vehicles
			 (Owner, birth_day, nationality, birth_place, mother_name, mobile_no, email, address, gender, personal_id, 
			  ...
			  v_type, color, hp, passengers, chassis_no, model, cylinder, comments, issue_place, created_At, status, receipt_no, username)
			 values
			 ('".mysqli_real_escape_string($con, $_POST['owner'])."',
			  ...
			  '".mysqli_real_escape_string($con, $_POST['passengers'])."',
			  ...
			  '".date('Y-m-d')."',
			  1,
			  '".mysqli_real_escape_string($con, $_POST['search_receipt_no'])."',
			  '".$_SESSION['USERNAME']."'
			 )
			 ";
			...
			if(mysqli_query($con, $sql))
			{
			 ...
			 mysqli_query($con, "update tbl_receipts
			   ...
			   $sql = "INSERT INTO tbl_v_cardlist (plate_No,owner,nationality, date_issue, place_issued, expire_date,model,color,vehicle_Type,origin,passengers,chassis_no) 
						...
						FROM tbl_vehicles 
						WHERE tbl_vehicles.plate_No ='".(mysqli_real_escape_string($con, $_POST['code']).''.mysqli_real_escape_string($con, $_POST['plate_no']))."'
					 ";	
				mysqli_query($con,$sql);
				//echo $sql; exit;	 				 
...
{   // cl_owner    cl_nationality   cl_issue_date cl_expire_date  cl_plate_no  cl_origin cl_v_type  cl_color
	...
	$sql = "INSERT into tbl_vehicles
			 (Owner, nationality, issue_date, expire_date, plate_no,  origin,  
			  v_type, color,  chassis_no, model, issue_place, passengers, created_at, status)
			 values
...
else if(isset($_POST['action']) && $_POST['action'] == 'vehicles_cardlist_ajax_pagination' && isset($_POST['page']))
{
	...
	$pagination_data = $_SESSION['vehicle_card_pagination_data'];
	$startpoint = ($page * $per_page) - $per_page;
	...
	while($rec = mysqli_fetch_object($data))
	{  
	   if(isset($user_privileges[2]) && $user_privileges[2]['e'] == 1)
	   {
		   $edit_btns = '<a href="?cardlist_id='.($rec->plate_no).'" class="glyphicon glyphicon-edit"></a>';
	   }
	   else
	       $edit_btns = '';
	   $result .= '<tr>
					...
					<td>'.$rec->owner.'</td>
					<td>'.$rec->nationality.'</td>
					<td>'. date('d/m/Y', strtotime($rec->date_issue)).'</td>
					<td>'.$rec->place_issued.'</td>
...
else if(isset($_POST['s_v_name']) || isset($_POST['s_v_number']) || isset($_POST['v_type']) || isset($_POST['v_contact']) || isset($_POST['v_chassis']))      // Search records
{
	$where = '';
	...
	if(isset($_POST['v_chassis']) && $_POST['v_chassis'] != '')
	{
	   if($where != '')
	      $where .=' AND ';	
	   $where .= ' chassis_no LIKE "%'.mysqli_real_escape_string($con, $_POST['v_chassis']).'%" ';  
	}
...
else if(isset($_POST['s_v_card_name']) || (isset($_POST['s_v_card_from_date']) && isset($_POST['v_card_to_date'])))      // Search records
{
	...
	if(isset($_POST['s_v_card_from_date']) && $_POST['s_v_card_from_date'] != '' && isset($_POST['v_card_to_date']) && $_POST['v_card_to_date'] != '')
	{
	   if($where != '')
	      $where .=' AND ';	
	   $where .= ' (Date(created_datetime) between "'.date('Y-m-d', strtotime(mysqli_real_escape_string($con, $_POST['s_v_card_from_date']))).'" and "'.date('Y-m-d', strtotime(mysqli_real_escape_string($con, $_POST['v_card_to_date']))).'" )';  
	   /*$where .= ' ( v.issue_date Between "'.date('Y-m-d', strtotime($_POST['s_v_card_from_date'])).'" and "'.date('Y-m-d', strtotime($_POST['v_card_to_date'])).'" 
					  OR vr.renewal_date Between "'.date('Y-m-d', strtotime($_POST['s_v_card_from_date'])).'" and "'.date('Y-m-d', strtotime($_POST['v_card_to_date'])).'" 
					  OR DATE(vr.created_date) Between "'.date('Y-m-d', strtotime($_POST['s_v_card_from_date'])).'" and "'.date('Y-m-d', strtotime($_POST['v_card_to_date'])).'" 
					)	 
					';*/
	}
	...
	{
		// statement for pagination
		//$statement = ' tbl_vehicles where'.$where.' order by id desc';
		...
		//echo "SELECT * FROM {$statement} LIMIT {$startpoint} , {$per_page}"; exit;
		$user_privileges = $_SESSION['logged_in_user_privileges'];
		//echo '<pre>'; print_r($user_privileges); exit;
		while($rec = mysqli_fetch_object($data))
		{  
		   if(isset($user_privileges[2]) && $user_privileges[2]['e'] == 1)
		   {
...
else if( isset($_POST['action']) &&  $_POST['action'] == 'Remove_session_value_of_search_statement')
{
	...
	unset($_SESSION['search_cl_statement']);
	return true;
...
else if( isset($_POST['action']) &&  $_POST['action'] == 'get_new_vehicle_plate_number')
{
	...
	// get digit number of plate code
	$sql = "select * from tbl_vehicle_plate_codes where vehicle_plate_code = '".$_POST['code']."' ";
	$plate_data = mysqli_query($con, $sql);
	$plate_rec = mysqli_fetch_object($plate_data);
	echo json_encode(array('status' => 'yes', 
						   'NewPlateNo' => $rec->NewPlateNo,
						   'plate_no_digits' => $plate_rec->digits));
